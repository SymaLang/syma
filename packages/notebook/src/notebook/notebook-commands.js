...
import { Sym, Str, Num, Call, isSym, isCall, isStr } from '@syma/core/ast-helpers';
import * as engine from '@syma/core/engine';
import { foldPrims } from '@syma/core/primitives';
import { DOMProjector } from '@syma/projectors';
import { NotebookModulePlatform } from './notebook-module-platform.js';
import { ModuleCompiler } from '@syma/core/module-compiler';

export class NotebookCommands {
    constructor(repl) {
        this.repl = repl;
        this.moduleIndex = null; // Will be loaded lazily
        this.originalCommandProcessor = null; // Will be set by notebook-engine
        ...
        switch (cmd) {
            ...
            case 'm':
                ...
                if (matchArgs === '') {
                    ...
                    if (this.originalCommandProcessor) {
                        return this.originalCommandProcessor.processCommand(command);
                    }
                    return true;
                }
            case 'trace':
            ...
            default:
                // Delegate to original REPL command processor for all other commands
                if (this.originalCommandProcessor) {
                    return this.originalCommandProcessor.processCommand(command);
                } else {
    ...
    async processInlineTrace(exprText, verbose = false, diff = false) {
        ...
        try {
            ...
            if (isNotebook && this.repl.platform.printStructured && trace && trace.length > 0) {
                ...
                const { collapseConsecutiveRules, formatCollapsedTrace, getTraceStats } = await import('@syma/core/core/trace-utils.js');
    ...
    async importNotebookModule(moduleName, modifiers = {}) {
        ...
        try {
            ...
            const { Module } = await import('@syma/core/module-compiler');
