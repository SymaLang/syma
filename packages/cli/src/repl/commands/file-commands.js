...
import { isCall, isSym, isStr } from '@syma/core/ast-helpers';
...
export class FileCommands {
    ...
    async bundle(args, rawArgs) {
        ...
        try {
            ...
            // The compiler will automatically discover and load dependencies
            // We just need to pass the entry file
            const command = `syma-compile "${filename}" --bundle --entry "${moduleName}"`;
            ...
            try {
                ...
                throw new Error(`Compilation failed (exit code ${error.status}):\nSTDOUT: ${stdout.substring(0, 500)}\nSTDERR: ${stderr.substring(0, 500)}`);
            }

            if (!result || result.trim().length === 0) {
                throw new Error('Compiler produced no output. Command: ' + command);
            }
    ...
    async import(args, rawArgs) {
        ...
        try {
            ...
            if (stdlibModules[moduleName]) {
                // It's a stdlib module - resolve to file path (monorepo structure)
                filename = `packages/stdlib/syma/${stdlibModules[moduleName]}`;
            ...
            const { execSync } = await import('child_process');
            const command = `syma-compile ${filename} --library`;
