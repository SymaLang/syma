...
import { compile as symaCompile } from '@syma/cli/compiler';
...
export default function symaPlugin(options = {}) {
    ...
    async function scanModules() {
        const patterns = [
            ...
            path.join(rootDir, 'packages/stdlib/syma', '**/*.syma')  // Also scan stdlib
        ];
    ...
    async function compileModules(entryName = entryModule) {
        ...
        console.log(`[syma] Bundling with entry ${entryName}...`);

        try {
            const result = await symaCompile({
                files: moduleFiles,
                bundle: true,
                entry: entryName,
                pretty,
            ...
            // Count actual bundled modules from the result
            const universe = JSON.parse(result);
            const bundledModuleCount = universe.Modules ? Object.keys(universe.Modules).length : moduleFiles.length;
            console.log(`[syma] ✓ Bundled ${bundledModuleCount} modules`);
